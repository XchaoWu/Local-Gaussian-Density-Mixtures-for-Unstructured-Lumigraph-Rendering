    ...
    int batch_size)
{
    ...
    while(taskIdx < batch_size)
    {
        ...
        // normalized points  this pts already normalized, must inside [-2,2]
    ...
    int batch_size)
{
    ...
    while(taskIdx < batch_size)
    {
        ...
        float3 doffset_dpts = make_float3(level_resolution-1) / 4.0f;
        ...
        atomicAdd(&grad_points[taskIdx].x, doffset_dpts.x * dot(dL_dout, dout_dx));
        atomicAdd(&grad_points[taskIdx].y, doffset_dpts.y * dot(dL_dout, dout_dy));
        atomicAdd(&grad_points[taskIdx].z, doffset_dpts.z * dot(dL_dout, dout_dz));
