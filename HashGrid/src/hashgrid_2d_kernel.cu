...
void encoding_forward_kernel(
    ...
    float2* features,  // K x Levels x (2**hashmap_size) x 2 
    int n_levels,
    int hashmap_size,
    int2* resolutions, 
    int batch_size)
{
    ...
    while(taskIdx < batch_size)
    {
        ...
        if (pts.x < 0 || pts.x > 1 || pts.y < 0 || pts.y > 1) 
        {
            taskIdx += total_thread;
            continue;
        }

        int hashidx = hash_idxs[taskIdx]; // the hash grid idx related to view idx 
        ...
        float2* level_feature = features + (hashidx * n_levels + blockIdx.y) * hashmap_size;
...
void encoding_forward_cuda(
    ...
    at::Tensor hash_idxs, 
    at::Tensor &outputs,
    at::Tensor features, // K x Levels x (2**hashmap_size) x 2
    at::Tensor resolutions)
{
    int batch_size = points.size(0);
    int n_levels = features.size(1);
    int hashmap_size = features.size(2);
    ...
    encoding_forward_kernel<<<dim3(NUM_BLOCK(batch_size), n_levels), NUM_THREAD>>>(
        ...
        (int*)hash_idxs.contiguous().data_ptr<int>(),
...
void encoding_backward_kernel(
    ...
    float2* grad_features, // k x Levels x hashmap_size x 2
    float2* features, // k x Levels x hashmap_size x 2
    ...
    int batch_size)
{
    ...
    while(taskIdx < batch_size)
    {
        ...
        float2* level_feature = features + (hashidx * n_levels + blockIdx.y) * hashmap_size;
        float2* level_grad_features = grad_features + (hashidx * n_levels + blockIdx.y) * hashmap_size;
