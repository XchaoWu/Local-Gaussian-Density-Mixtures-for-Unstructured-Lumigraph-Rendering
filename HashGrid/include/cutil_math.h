...
static __inline__ __host__ __device__ float3 fmaxf(float3 a, float b)
{
...
inline __host__ __device__ float3 operator+=(float3 a, uint32_t b)
{
    return make_float3(a.x + (int)b, a.y + (int)b, a.z + (int)b);
}
...
inline __host__ __device__ int3 operator*(int3 a, uint32_t s)
{
    return make_int3(a.x * (int)s, a.y * (int)s, a.z * (int)s);
}
...
inline __device__ __host__ int3 clamp(int3 v, int a, int3 b)
{
    return make_int3(clamp(v.x, a, b.x), clamp(v.y, a, b.y), clamp(v.z, a, b.z));
}
...
inline __host__ __device__ uint3 make_uint3(int3 s)
{
    return make_uint3((uint32_t)s.x, (uint32_t)s.y, (uint32_t)s.z);
}
...
__device__ inline uint3 toInt(float3 xyz)
{
    return make_uint3((uint32_t)xyz.x,(uint32_t)xyz.y,(uint32_t)xyz.z);
}
