...
#     factor = 1
  ...
  oridir = os.path.join(basedir, 'images')
  imgdir = os.path.join(basedir, 'images' + sfx)
  
  if os.path.exists(imgdir) is False:
    os.mkdir(imgdir)
    imgs = [os.path.join(oridir, f) for f in sorted(os.listdir(oridir))]
    imgs = [f for f in imgs if any([f.endswith(ex) for ex in ['JPG', 'jpg', 'png', 'jpeg', 'PNG']])]
    for f in imgs:
        basename = os.path.basename(f)
        newimg = cv2.resize(cv2.imread(f), (width, height), interpolation=cv2.INTER_AREA)
        cv2.imwrite(os.path.join(imgdir, basename), newimg)

  h_factor = sh[0] / height
  w_factor = sh[1] / width
  intrinsic_arr[2, 0] /= w_factor
  intrinsic_arr[3, 0] /= h_factor
  intrinsic_arr[4, 0] /= w_factor
  intrinsic_arr[5, 0] /= h_factor
  #h, w, fx, fy, cx, cy
#   intrinsic_arr = intrinsic_arr / factor
...
def load_llff_data(basedir, height=756, width=1008, factor=8, recenter=True, bd_factor=.75, 
                   spherify=False, path_zflat=False, split_train_val = 0, render_style=''):

  
  
  # poses, bds, imgs = _load_data(basedir, factor=factor) # factor=8 downsamples original imgs by 8x
  poses, bds, imgs, intrinsic = _load_data(basedir, width=width, height=height, load_imgs=True) # factor=8 downsamples original imgs by 8x
  ...
  if split_train_val == 0:
    ...
    return images, poses, bds, render_poses, i_test, intrinsic, dmin, dmax
  else:
