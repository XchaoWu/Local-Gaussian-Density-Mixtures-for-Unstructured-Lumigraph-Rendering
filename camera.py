...
def get_center_and_ray_v3(H,W,pose,intr=None, ray_idx=None): # [HW,2]
    # given the intrinsic/extrinsic matrices, get the camera center and ray directions]
    # assert(opt.camera.model=="perspective")
    with torch.no_grad():
        # compute image coordinate grid
        y_range = torch.arange(H,dtype=torch.float32,device=pose.device)
        x_range = torch.arange(W,dtype=torch.float32,device=pose.device)
        Y,X = torch.meshgrid(y_range,x_range) # [H,W]
        xy_grid = torch.stack([X,Y],dim=-1).view(-1,2) # [HW,2]
    
    """
    ray_idx  [ [5,4,3], [1,2,6], ...] B x num_sampled_rays_per_view
    """
    ...
    if ray_idx != None:
        # temp = torch.arange(batch_size, dtype=torch.long, device=pose.device)[:,None].repeat(1,ray_idx.shape[1])
        xy_grid = xy_grid[ray_idx[0],ray_idx[1],:].reshape(batch_size,-1,2)
...
def get_center_and_ray_v2(H,W,pose,intr=None, ray_idx=None): # [HW,2]
    # given the intrinsic/extrinsic matrices, get the camera center and ray directions]
    # assert(opt.camera.model=="perspective")
    with torch.no_grad():
        # compute image coordinate grid
...
def get_center_and_ray(H,W,pose,intr=None): # [HW,2]
    # given the intrinsic/extrinsic matrices, get the camera center and ray directions]
    # assert(opt.camera.model=="perspective")
    with torch.no_grad():
        # compute image coordinate grid
...
def get_center_and_ray_local(H,W,ray_idx=None, intr=None): # [HW,2]
    # given the intrinsic/extrinsic matrices, get the camera center and ray directions]
    # assert(opt.camera.model=="perspective")
    with torch.no_grad():
        # compute image coordinate grid
        y_range = torch.arange(H,dtype=torch.float32,device=intr.device)
        x_range = torch.arange(W,dtype=torch.float32,device=intr.device)
        Y,X = torch.meshgrid(y_range,x_range) # [H,W]
        xy_grid = torch.stack([X,Y],dim=-1).view(-1,2) # [HW,2]
    ...
    batch_size = len(intr)
    xy_grid = xy_grid.repeat(batch_size,1,1) # [B,HW,2]
    ray = img2cam(to_hom(xy_grid),intr) # [B,HW,3]
    return ray 
